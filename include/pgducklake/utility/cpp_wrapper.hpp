...
__CPPFunctionGuard__(const char *func_name, const char *file_name, int line,
                     FuncArgs &...args) {
  ...
  // If we are here, it means we got out of the PG_TRY block through a C++
  // exception without running the corresponding `PG_CATCH` block.
  //
  // In that case, the `PG_exception_stack` points to an invalid stack and
  // calling `elog(ERROR, ...)` below would lead to a crash when we finally hit
  // `PG_CATCH`
  //
  // As a best effort, we reset the `PG_exception_stack` to the value we had
  // entering the wrapper, but this might leak other resources that were
  // supposed to be closed in the missed `PG_CATCH`
  //
  ...
  if (pg_es_start != PG_exception_stack) {
    elog(WARNING, "WARNING: Unexpected exception stack pointer. This is not "
                  "expected, please report this.");
...
#define InvokeCPPFunc(FUNC, ...)                                               \
  pgducklake::__CPPFunctionGuard__<decltype(&FUNC), &FUNC>(                    \
      #FUNC, __FILE__, __LINE__, ##__VA_ARGS__)
